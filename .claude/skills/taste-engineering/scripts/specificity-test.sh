...
grep -rn --include="*.tsx" --include="*.jsx" \
  ...
  "$TARGET_DIR" 2>/dev/null | grep -v node_modules | grep -v '.claude/' | head -20
echo ""
...
grep -rn --include="*.tsx" --include="*.jsx" --include="*.html" \
  ...
  "$TARGET_DIR" 2>/dev/null | grep -v node_modules | grep -v '.claude/' | head -15
echo ""
...
find "$TARGET_DIR" -type f \( -name "Card.tsx" -o -name "Hero.tsx" -o -name "Section.tsx" \
  ...
  ! -path "*/node_modules/*" ! -path "*/.claude/*" 2>/dev/null | head -10
echo ""
...
GENERIC_COUNT=$(grep -rl --include="*.tsx" --include="*.jsx" --include="*.ts" --include="*.js" \
  ...
  "$TARGET_DIR" 2>/dev/null | grep -v node_modules | grep -v '.claude/' | wc -l | tr -d ' ')
